...
export function subscribeMedicines(userId, callback) {
  ...
  return onSnapshot(q,
    ...
    (err)  => { console.error('[Firestore] subscribeMedicines error:', err.code); callback([]); }
  );
}
...
export async function deleteMedicine(userId, medId) {
  ...
  logsSnap.docs.forEach((doseLog) => batch.delete(doseLog.ref));
  batch.delete(doc(db, 'users', userId, 'medicines', medId));
...
export async function logDose(userId, doseLog) {
  ...
  const medRef = doc(db, 'users', userId, 'medicines', medicineId);
  ...
  // Update points if taken on time (use setDoc merge so it works even if user doc is missing)
  if (status === 'taken') {
    await setDoc(doc(db, 'users', userId), { points: increment(10) }, { merge: true }).catch(() => {});
  }

  // Decrement pill count if tracked
  if (status === 'taken') {
    const medSnap = await getDoc(medRef).catch(() => null);
    const pillCount = medSnap?.exists() ? medSnap.data().pillCount : null;
    if (typeof pillCount === 'number' && pillCount > 0) {
...
export function subscribeDoseLogs(userId, date, callback) {
  ...
  return onSnapshot(q,
    (snap) => { callback(snap.docs.map((d) => ({ id: d.id, ...d.data() }))); },
    (err)  => { console.error('[Firestore] subscribeDoseLogs error:', err.code); callback([]); }
  );
}
